...
async function main() {
  ...
  const filePath = path.join(process.cwd(), 'data', 'taco.csv');

  if (!fs.existsSync(filePath)) {
    console.error('❌ Arquivo taco.csv não encontrado em ./data/');
    process.exit(1);
  }
  ...
  for (const row of records) {
    ...
    try {
      const existing = await prisma.food.findFirst({
        where: { description, sourceTable: 'TACO', deletedAt: null },
      });
      ...
      if (existing) {
        ...
        const newFood = await prisma.food.create({
          data: { description, groupName, sourceTable: 'TACO', portionGrams: 100 },
        });
        foodId = newFood.id;
        imported++;
      }
